...
- (void) startScanning {
    ...
    [self reportDelegateWithStatus:(LQRScannerWillStartSession)];
    ...
    dispatch_async(queue, ^{
        ...
        if (self.session.isRunning == NO) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self reportDelegateWithStatus:(LQRScannerDidStartSession)];
            });
        }
    });
}
...
- (void) observerSelectors:(NSNotification *) noti {
    if (noti.name == UIApplicationDidEnterBackgroundNotification) {
        [self reportDelegateWithStatus:LQRScannerDidEnterBackground];
    } else if (noti.name == UIApplicationWillEnterForegroundNotification) {
        [self reportDelegateWithStatus:LQRScannerWillEnterForeground];
        ...
        if ([object isKindOfClass:[AVCaptureSession class]]) {
            ...
            if ([keyPath isEqualToString:@"running"]) {
                ...
                if (isRunning) {
                    ...
                    [self reportDelegateWithStatus:LQRScannerStartScanning];
                    
                }else{
//                    [self reportDelegateWithStatus:LQRScannerStopScanning];
                }
            }
        }
    });
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        for (AVMetadataMachineReadableCodeObject *obj in metadataObjects) {
            LQRScanItem *res = [[LQRScanItem alloc]init];
...
- (void) resetSession {
    ...
    if (self.isAutoStartScan) {
        [self startScanning];
    } else {
        [self reportDelegateWithStatus:LQRScannerPrepareDone];
    }
}
...
- (void) reportDelegateWithStatus:(LQRScannerStatus) status {
    
//    dispatch_async(dispatch_get_main_queue(), ^{
//
//        if (self.delegate && [self.delegate respondsToSelector:@selector(scanner:statusChanged:)]) {
//            [self.delegate scanner:self statusChanged:status];
//        }
//    });
