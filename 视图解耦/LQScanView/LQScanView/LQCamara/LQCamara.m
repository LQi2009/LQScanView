...
@interface LQCamara () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCapturePhotoCaptureDelegate> {
...
- (void) prepareSession {
    ...
    if (@available(iOS 10.0, *)) {
        ...
        [po capturePhotoWithSettings:settings delegate:self];
        [po connectionWithMediaType:AVMediaTypeVideo];
        if ([self.session canAddOutput:po]) {
            [self.session addOutput:po];
        }
    } else {
